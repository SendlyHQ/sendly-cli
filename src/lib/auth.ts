...
export async function browserLogin(): Promise<TokenResponse> {
  ...
  // Enable press-to-copy UX while we poll. Only wire this when we have a
  // real interactive TTY — in CI or piped stdin there's nothing to read
  // from and setRawMode throws. The keypress listener is detached on any
  // exit path (success, timeout, error) so we don't leave the terminal in
  // raw mode.
  const keypressCleanup = attachLoginKeypressHandler(data.verificationUrl);
...
/**
 * Wire up the copy-URL / cancel keybindings during the device-code
 * polling loop. Only active on a real interactive TTY — in CI, piped
 * stdin, or when stdin.setRawMode is missing we no-op and return a
 * cleanup that also no-ops.
 *
 ...
 */
function attachLoginKeypressHandler(urlToCopy: string): () => void {
